...
const BookCard = ({ image, title, author, price, bookid, fav, onRemove  }) => {
  const headers = {
    bookid: bookid,
  ...
  const removeFromFavourite = async () => {
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_API_URL}/api/v1/remove-from-favourite`,
        {},
        { headers }
      );

      if (onRemove) {
        onRemove(bookid); // Call parent's remove function
      }
